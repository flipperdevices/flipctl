use std::sync::Arc;
use axum::{Router, extract::{State, ws::{WebSocketUpgrade, WebSocket, Message}}, response::IntoResponse, Json};
...
pub async fn serve(port: u16, frame_tx: broadcast::Sender<Frame>, key_tx: mpsc::Sender<RemoteKey>, width: u32, height: u32) {
    let state = AppState { frame_tx, key_tx, width, height };
    let app = Router::new()
...
<head><meta charset="utf-8"><title>servo-remote</title>
<style>
  * { margin: 0; }
...
async fn ws_handler(
    ...
    ws.on_upgrade(move |socket| ws_stream(socket, rx, w, h))
}

async fn ws_stream(mut socket: WebSocket, mut rx: broadcast::Receiver<Frame>, w: u32, h: u32) {
    // First message: dimensions so the browser can set up ImageData
    let init = format!("{{\"w\":{w},\"h\":{h}}}");
    if socket.send(Message::Text(init)).await.is_err() { return; }
...
async fn mjpeg_handler(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let boundary = "servo_frame";
    let stream = async_stream::stream! {
        loop {
            match rx.recv().await {
                Ok(frame) => {
                    let hdr = format!("--{boundary}\r\nContent-Type: image/png\r\nContent-Length: {}\r\n\r\n", frame.len());
