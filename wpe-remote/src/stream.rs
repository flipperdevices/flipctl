...
pub async fn serve(port: u16, frame_tx: broadcast::Sender<Frame>, key_tx: mpsc::Sender<RemoteKey>) {
    ...
    let app = Router::new()
        ...
        .route("/stream", axum::routing::get(mjpeg_handler)) // keep MJPEG as fallback
...
<head><meta charset="utf-8"><title>wpe-remote</title>
<style>
  * { margin: 0; }
  ...
  <script>
    ...
    // WebSocket stream — no browser buffering unlike MJPEG
    const ws = new WebSocket('ws://' + location.host + '/ws');
    ws.binaryType = 'arraybuffer';
    let drawing = false;
    ws.onmessage = async (e) => {
      if (drawing) return; // skip if previous frame still decoding
      drawing = true;
      try {
...
async fn ws_handler(
    ...
    ws.on_upgrade(|socket| ws_stream(socket, rx))
}
